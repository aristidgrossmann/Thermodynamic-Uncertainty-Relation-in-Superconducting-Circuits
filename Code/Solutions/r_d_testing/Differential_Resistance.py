...
from Exact_Solutions import L, r_d, I_tilde_plus_minus, Uncertainty_product, T_1_paper, Mean_Velocity, Variance_Velocity
...
for i in range(len(D_vals)):
    ...
    for j in range(len(i_0_vals)):
        ...
        r_d_analytical[j] =  2*r_d(i_0 = i_0_vals[j], D = D_vals[i])#*i_0_vals[j]/Mean_Velocity(i_0 = i_0_vals[j], D = D_vals[i])

    ax.plot(i_0_vals, r_d_analytical, label=D_labels[i] + ' r_d', color = colors[i+1])
    ax.scatter(i_0_vals, Uncertainty_Product_vals, label=D_labels[i] + ' TUR', color = 'red')
...
plt.tight_layout()
plt.legend()
plt.savefig('Differential Resistance TUR Testing.pdf')
plt.show()
