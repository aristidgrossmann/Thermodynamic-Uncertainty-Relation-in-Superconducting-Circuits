...
for i in range(len(D_vals)):
    ...
    ax1.plot(i_0_vals, Uncertainty_product_vals, linestyle = '-', label = D_labels[i], color = colors[i], zorder = 1)
...
for i in range(len(D_vals)):
    ...
    # ax.plot(i_0_vals, Uncertainty_product_vals, label=D_labels[i], linestyle = '-', color = colors[i], zorder = 1)
    ax2.plot(i_0_vals, Uncertainty_product_vals, linestyle = '-', label = D_labels[i], color = colors[i], zorder = 1)
...
ax2.text(0.45, 0.6, r'$\tilde{\mathcal{Q}}$', transform=ax2.transAxes,
         fontsize=11, va='top', ha='left')

ax2.text(0.35, 0.6, r'$\tilde{\mathcal{L}}$', transform=ax2.transAxes,
         fontsize=11, va='top', ha='left')
...
plt.savefig('TUR_Ratchet_a_0_L.svg')
plt.show()
